...
```
> ⚠️ **La clé n'est jamais écrite dans le repo.** `.mcp.json` est versionné : on n'y met que
> l'**expansion** `${MESHY_API_KEY}`, et chaque machine exporte sa propre clé dans son environnement.
> Une clé collée en dur ici finirait dans l'historique Git. Cf `docs/tech/meshy_setup.md`.
...
## 5. Stratégie modèles IA (Opus / Fable) — **révisée**

> **Révision actée (2026-08-07).** La première version de cette section se contredisait : elle envoyait à Fable
> le **code le plus dur** (`boss_ai`, `combo_engine`, l'algorithmique du navmesh, les passes de review technique)
> tout en affirmant que Fable était le spécialiste *créatif*. Corrigé ci-dessous. **Tout le code part sur Opus.**

Deux modèles, deux rôles — la frontière est **code vs création**, pas *facile vs difficile* :

| | ● Opus (défaut) | ◆ Fable (réservé, mesuré) |
|---|---|---|
| Statut | inclus dans la souscription → gratuit au point d'usage | budget plafonné (~100 $) |
| Rôle | **100 % du code et de l'exécution** : GDScript, **Rust (`boss_ai`, `combo_engine`)**, algorithmique (navmesh sur relief), MCP Godot, MCP Meshy, `.tres`, tests, build, docs techniques, **review de code** | **Créatif pur, sans code** : conception géo/topographique, direction artistique, rythme narratif de l'onboarding, design des rencontres, **review artistique** |
| Bonus | Fast mode (sortie plus rapide) | pas de fast mode |

**Règles :**
- **On démarre toujours sur Opus**, et on n'en sort que pour une tâche explicitement marquée `◆ FABLE`.
- **Fable ne produit jamais de code ni d'édition de scène.** Sa sortie est un *document* (spec, carte, script,
  liste de retouches) assez précis pour qu'Opus l'exécute sans réinterprétation. C'est le contrat d'interface
  entre les deux modèles, et c'est ce qui rend le burn Fable faible : peu d'output.
- **Créatif géo/topo = Fable.** Silhouette de la caverne, relief, sightlines, puits de ciel, landmarks, logique
  de « où va le joueur et pourquoi ».
- **Borne chaque run Fable** (`effort: high`, cahier des charges complet en entrée) — c'est l'output qui coûte.
- **Sessions dédiées** : ne pas alterner Opus↔Fable dans une même session (cache invalidé, contexte perdu).
- **Jauge le burn sur la première session Fable** pour caler le budget des suivantes.

### 5.1. Séquencement — créatif et exécution s'alternent, avec **une boucle de retour**

La première version regroupait le travail en 5 sessions mono-modèle en **front-loadant tout le créatif** dans la
Session 1, sans jamais y revenir. **Corrigé** : le level design est itératif, et une topographie conçue à l'aveugle
ne survit jamais intacte au premier playtest. On garde le principe des sessions mono-modèle (le coût de bascule est
réel), mais on **prévoit explicitement un aller-retour créatif après le blockout**.

| # | Session | Modèle | Regroupe | Produit |
|---|---|---|---|---|
| 1 | **Socle & outillage** | ● Opus | docs sur `main` · `.mcp.json` Meshy · licence · manifest · **probe E1 sur 1 asset** · bench budget E0 | boucle Meshy prouvée + budget mesuré |
| 2 | **Direction créative** | ◆ Fable | **E2 conception géo/topographique** (brief art bible §6bis) | spec topographique en coordonnées |
| 3 | **Construction** | ● Opus | E2 terrain + blockout MCP Godot + étanchéité + navmesh + portage systèmes | caverne grise jouable |
| 3b | **Playtest** | 🎮 humain | 2-4 joueurs sur le blockout | rapport de playtest |
| 4 | **Retour créatif** | ◆ Fable | **review de la caverne construite** (screenshots + playtest) → corrections topo | patch de la spec |
| 5 | **Matière** | ● Opus | E3 texturing/éclairage/brume/perf · E4 assets & props | caverne texturée |
| 6 | **Rythme** | ◆ Fable | E5 mise en scène des 8 beats · E6 design rencontres/tension/révélation boss | scripts de mise en scène |
| 7 | **Gameplay** | ● Opus | E5 antichambre · E6 **`boss_ai` (Rust)** + wiring encounters/loot/puzzle · E7 audio/perf/bugs | run complète |
| 8 | **Review artistique** | ◆ Fable | E7 regard DA final sur caverne + boss | liste de retouches |
| 9 | **Remise** | ● Opus | retouches · build · script de démo | cadeau livrable |

**Logique** : chaque session Fable **précède** l'exécution Opus qu'elle alimente, et la session 4 ferme la boucle que
la version initiale avait oubliée. Budget Fable réparti sur 4 sessions courtes et bornées (2, 4, 6, 8).
...
**Modèle : ◆ Fable pour la conception → ● Opus pour toute l'exécution.** **Fable conçoit la structure géographique et topographique** : plan de la caverne, sculpture du relief vallonné, sightlines, composition des landmarks et des puits de ciel, placement intentionnel des 4–6 POI. **Opus exécute** : choix technique du terrain, blockout via MCP Godot, étanchéité du volume, **et l'algorithmique du navmesh sur relief** (c'est du code, donc Opus — corrigé par rapport à la v1 de ce plan). *(C'est l'étape où le budget Fable travaille le plus.)*
Puis **boucle de retour** : après le playtest du blockout, Fable revoit sa topo confrontée au réel et produit des corrections, qu'Opus applique.
...
**Modèle : ◆ Fable pour le design → ● Opus pour tout le code.** **Fable** conçoit les rencontres, la courbe de tension et la mise en scène de la révélation du boss (adossée aux sightlines d'E2). **Opus** écrit le **Rust `boss_ai`** (corrigé : le Rust dur ne part pas sur Fable), câble les encounters, le placement, le loot et le verrouillage d'arène.
...
**Modèle : ● Opus + une passe de review ◆ Fable.** Opus pour audio/post-process/perf/bugs/build **et la review de code**. **Fable** pour une **review artistique finale** (regard DA sur caverne + boss) avant la remise.
